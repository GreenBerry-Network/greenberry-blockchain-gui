...
const PY_FOLDER = '../greenberry/daemon';
...
const getGreenberryVersion = () => {
  let version = null;
  const exePath = getExecutablePath('greenberry');
  // first see if we can get a greenberry exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or greenberry is on the path
    try {
      version = child_process
...
module.exports = {
  startGreenberryDaemon,
  getGreenberryVersion,
  guessPackaged,
};
