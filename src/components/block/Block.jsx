...
import { Card, FormatLargeNumber, Link, Loading, TooltipIcon, Flex } from '@greenberry/core';
import {
  unix_to_short_date,
  hex_to_array,
  arr_to_hex,
...
export default function Block() {
  ...
  const poolReward = mojo_to_greenberry(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_greenberry(calculateBaseFarmerReward(blockRecord.height));

  const greenberryFees = blockRecord.fees
    ? mojo_to_greenberry(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: greenberryFees ? `${greenberryFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    >
      <Card
        title={(
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Greenberry
              blockchain
            </Trans>
          </BlockTitle>
        )}
