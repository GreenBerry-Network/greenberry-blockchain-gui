...
import { AlertDialog, Card, Flex } from '@greenberry/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const greenberryper_input = ip_parsed.limit;
    ...
    const greenberryper_value = Number.parseInt(Number(greenberryper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (greenberry per interval):{' '}
                  {mojo_to_greenberry_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
