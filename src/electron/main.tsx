...
import greenberryEnvironment from '../util/greenberryEnvironment';
import greenberryConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!greenberryEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    greenberryConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!greenberryConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !greenberryConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (greenberryConfig.manageDaemonLifetime()) {
        greenberryEnvironment.startGreenberryDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Greenberry Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Greenberry-Network/greenberry-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Greenberry-Network/greenberry-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Greenberry-Network/greenberry-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Greenberry-Network/greenberry-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Greenberry-Network/greenberry-blockchain/issues',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal('https://twitter.com/greenberry_project');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Greenberry Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Greenberry' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Greenberry Blockchain' }),
            click: () => {
              openAbout();
            },
          },
